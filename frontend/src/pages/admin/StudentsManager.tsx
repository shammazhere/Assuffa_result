...
const StudentsManager: React.FC = () => {
    ...
    const addStudent = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError(apiErr.response?.data?.error || 'Failed to add student. The USN may already exist.');
        }
    };

    const deleteStudent = async (id: string) => {
        if (!window.confirm('Are you sure? Deleting a student will also delete all their marks.')) return;
