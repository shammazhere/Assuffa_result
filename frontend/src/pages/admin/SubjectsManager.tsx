...
const SubjectsManager: React.FC = () => {
    ...
    const fetchFilteredSubjects = async (classId: string) => {
        try {
            const url = classId ? `/admin/subjects?class_id=${classId}` : '/admin/subjects';
    ...
    const addSubject = async (e: React.FormEvent) => {
        ...
        try {
            const response = await api.post('/admin/subjects', { name: newSubjectName, class_id: selectedClassId });
            // Add class info manually to avoid refetching if possible
            const createdWithClass = {
                ...response.data,
                class: classes.find(c => c.id === selectedClassId)
            };

            // If we are currently filtering by that class, or not filtering
            if (!filterClassId || filterClassId === selectedClassId) {
                setSubjects([...subjects, createdWithClass]);
            }
    ...
    const deleteSubject = async (id: string) => {
        if (!window.confirm('Are you sure? Deleting a subject will cascade and delete associated marks.')) return;
    ...
    return (
        <div className="bg-white rounded-xl shadow-lg border border-yellow-200 p-6 md:p-8">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 border-b-2 border-yellow-200 pb-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm font-bold text-gray-500 mt-1 uppercase tracking-wider text-black">Add, view, and remove subjects per class.</p>
                    </div>
                </div>

                <div className="flex items-center rounded-lg bg-yellow-50 border-2 border-yellow-200 p-1.5 shadow-sm">
                    <span className="px-3 text-xs font-black text-black uppercase tracking-widest">Filter:</span>
                    <select
                        value={filterClassId}
                        onChange={handleFilterChange}
                        className="bg-white px-3 py-1.5 rounded text-sm font-bold border-0 focus:ring-0 text-black shadow-sm outline-none cursor-pointer"
                    >
